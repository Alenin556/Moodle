...
public class WBPage {
    ...
    public SelenideElement chooseSize = $x("//*[contains(text(),\"Выберите размер\")]");
    ...
    public void addItemInBasket() {
        ...
        for (int i = 0; i < 2; i++) {
            if (i == 0) {
                ...
                try {
                    chooseSize.shouldNotBe(Condition.exist, Duration.ofSeconds(5));
                } catch (Exception ex) {
                    SelenideElement availableFirstSize = $x("//div[@class=\"popup__content\"]//li[1]");
